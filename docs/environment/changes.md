...
- **2026-05-16**: Controlled Process-mode execution from runtime approval panel implemented. Two explicit operator actions: Approve DryRun and Approve Process. Process mode loads ignored local config only after operator click. Dangerous flag detection prevents unsafe configs. DryRun remains available and safer. No automatic execution. No WebView result injection.
