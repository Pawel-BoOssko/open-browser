...
19. **Autonomous execution loop**: The LLM works autonomously with the local environment. It sends a command, the command executes, the result feeds back into the LLM conversation, and the LLM acts on that result in its next turn. There is no human in the runtime loop. The architecture is: LLM decides → executor executes → LLM receives the result. This is the core goal of the project.

20. **No dedicated Cloud Code Connector**: Cloud Code / Claude Code is a command-line tool invoked through the existing PowerShell/command-line executor or directly. It is not a separate architectural component. Do not create a dedicated connector, adapter, or integration layer just for Cloud Code. The general executor direction covers it.

21. **Result delivery to LLM is mandatory**: Execution results must flow back to the LLM conversation so it can act on them. This is a fundamental property of the autonomous loop, not an optional feature.
