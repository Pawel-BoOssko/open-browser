...
class Program
{
    ...
    static void Main()
    {
        ...
        var marker = "<<<OPENBRIDGE:EXEC:BEGIN>>>\n{\"version\":\"001\",\"command\":\"CC\"}\n<<<OPENBRIDGE:EXEC:END>>>";
        ...
        Assert(t1.Contains("<<<OPENBRIDGE:EXEC:BEGIN>>>"), "Complete flow preserves BEGIN marker");
        Assert(t1.Contains("<<<OPENBRIDGE:EXEC:END>>>"), "Complete flow preserves END marker");
        ...
        {
            CdpFrame(SseMessage("<<<OPENBRIDGE:EXEC:BEGIN>>> {\"version\":\"001\"} <<<OPENBRIDGE:EXEC:END>>>"))
        });
        Assert(t2.Contains("<<<OPENBRIDGE:EXEC:BEGIN>>>"), "Message-only preserves BEGIN");
        Assert(t2.Contains("<<<OPENBRIDGE:EXEC:END>>>"), "Message-only preserves END");

        // Test 3: Space-before->>> marker in full flow
        var markerSpaced = "<<<OPENBRIDGE:EXEC:BEGIN >>>\n{\"version\":\"001\"}\n<<<OPENBRIDGE:EXEC:END >>>";
        var t3 = RunWithCdpFrames(new[]
        {
            CdpFrame(SseMessage("Start.")),
            CdpFrame(SseDeltaReplace(markerSpaced))
        });
        Assert(t3.Contains("OPENBRIDGE:EXEC:BEGIN"), "Space-before->>> preserved in full flow");
        ...
        Assert(t4.Contains("<<<OPENBRIDGE:EXEC:BEGIN>>>"), "CRLF CDP preserves BEGIN");
        Assert(t4.Contains("<<<OPENBRIDGE:EXEC:END>>>"), "CRLF CDP preserves END");

        // Test 5: Markers survive unmodified — char-by-char verification
        var t5 = RunWithCdpFrames(new[]
        {
            CdpFrame(SseMessage("<<<OPENBRIDGE:EXEC:BEGIN>>>"))
        });
        var beginCount = CountSubstring(t5, "<<<OPENBRIDGE:EXEC:BEGIN>>>");
        var endCount = CountSubstring(t5, "<<<OPENBRIDGE:EXEC:END>>>");
        Assert(beginCount >= 1, $"BEGIN counted {beginCount} times (expected >=1)");
        Assert(!t5.Contains("<<OPENBRIDGE") || t5.Contains("<<<OPENBRIDGE"), "No missing angle bracket");
        ...
        Assert(t8.Contains("OPENBRIDGE:EXEC:BEGIN"), "Real log fragment preserves markers");
